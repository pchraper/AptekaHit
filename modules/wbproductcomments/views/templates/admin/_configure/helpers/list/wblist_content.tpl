            ...
            {foreach $fields_display AS $key => $params}
                ...
                {/if}
                    ...
                    {elseif isset($params.type) && $params.type == 'percent'}
                        {$tr.$key} {l s='%' mod='wbproductcomments'}
                ...
                <td class="text-right">
                    ...
                    {if Module::getInstanceByName('reviewsreply') != false}
                        {if isset(Module::getInstanceByName('reviewsreply')->active)}
                            {if Module::getInstanceByName('reviewsreply')->active == true && $identifier == 'id_wbproduct_comment'}
                                <div class="pull-right">
                                    <a onclick="$(this).closest('tr').next('tr').show();" class="btn btn-small btn-default"><i class="icon-pencil"></i> {l s='Reply' mod='wbproductcomments'}</a>
                                </div>
                            {/if}
                        {/if}
                    {/if}
                ...
                {if Module::getInstanceByName('reviewsreply')->active == true && $identifier == 'id_wbproduct_comment'}
                    <tr style="display:none;">
                        <td colspan="8" class="center" style="padding:8px">
                            {assign var='reply' value=rreply::getByIdWbProductCommentAll($tr.id_wbproduct_comment)}
                            <form method="post">
                                <input type="hidden" name="action" value="addreply"/>
                                <input type="hidden" name="id_wbproduct_comment" value="{$tr.id_wbproduct_comment}"/>
                                {if isset($reply.0)}
                                    {if isset($reply.0->body)}
                                        <input type="hidden" name="id_rreply" value="{$reply.0->id_rreply}"/>
                                    {/if}
                                {/if}
                                <textarea name="body" style="height:100px; margin-bottom:20px;" class="col-lg-12">{if Tools::getValue('action') == 'addreply' && Tools::getValue('id_wbproduct_comment') == $tr.id_wbproduct_comment}{Tools::getValue('body')}{elseif isset($reply.0)}{if isset($reply.0->body)}{$reply.0->body}{/if}{/if}</textarea>
                                <div class="col-lg-12">
                                    <button class="btn btn-large btn-success">
                                        <i class="icon-save"></i> {if isset($reply.0)}{l s='Save reply' mod='wbproductcomments'}{else}{l s='Add reply' mod='wbproductcomments'}{/if}
                                    </button>
                                </div>
                            </form>
                        </td>
    ...
    <tr>
        <td class="list-empty" colspan="{count($fields_display)+1}">
            <div class="list-empty-msg">
                ...
                {l s='No records found' mod='wbproductcomments'}
            </div>
        </td>
    </tr>
{/if}
