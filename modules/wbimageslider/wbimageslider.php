...
include_once(_PS_MODULE_DIR_.'wbimageslider/WbSlide.php');

class WbImageSlider extends Module implements WidgetInterface
{
    protected $html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('WB Custom Image slider', array(
            ), 'Modules.Imageslider.Admin');
        $this->description = $this->getTranslator()->trans('Adds an image slider to your site.', array(
            ), 'Modules.Imageslider.Admin');
        $this->ps_versions_compliancy = array('min' => '1.7.1.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:wbimageslider/views/templates/hook/slider.tpl';
    ...
    public function install()
    {
        ...
        if (parent::install() &&
            ...
            $res &= Configuration::updateValue('WBSLIDER_SPEED', $this->default_speed);
            $res &= Configuration::updateValue('WBSLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover);
            $res &= Configuration::updateValue('WBSLIDER_WRAP', $this->default_wrap);
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 2; ++$i) {
            ...
            foreach ($languages as $language) {
                ...
                $slide->description[$language['id_lang']] = '<h2>Latest Stylish Fashion 2019</h2><h5>Fashion Collection Get up to 20% off in this month</h5>';
                $slide->legend[$language['id_lang']] = 'slider-'.$i;
                $slide->url[$language['id_lang']] = 'http://www.prestashop.com/?utm_source=back-office&utm_medium=v17_wbslider'
                ...
                $slide->image[$language['id_lang']] = 'slider-'.$i.'.jpg';
            }
            $slide->add();
        }
    }
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('WBSLIDER_SPEED');
            $res &= Configuration::deleteByName('WBSLIDER_PAUSE_ON_HOVER');
            $res &= Configuration::deleteByName('WBSLIDER_WRAP');
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wbslider` (
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wbslider_slides` (
              `id_wbslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wbslider_slides_lang` (
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new WbSlide($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'wbslider`,
            `'._DB_PREFIX_.'wbslider_slides`,
            `'._DB_PREFIX_.'wbslider_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = WbSlide::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            if (!Validate::isInt(Tools::getValue('WBSLIDER_SPEED'))) {
            ...
            if (!Validate::isInt(Tools::getValue('active_slide')) ||
                ...
                $errors[] = $this->getTranslator()->trans('Invalid slide
                 state.', array(), 'Modules.Imageslider.Admin');
            }
            /* Checks position */
            ...
            if (Tools::isSubmit('id_slide')) {
                if (!Validate::isInt(Tools::getValue('id_slide')) &&
                    ...
                    $errors[] = $this->getTranslator()->trans('Invalid 
                        slide ID', array(), 'Modules.Imageslider.Admin');
                }
            }
            ...
            foreach ($languages as $language) {
                if (Tools::strlen(Tools::getValue('title_' . $language['id_lang'])) > 255) {
                    $errors[] = $this->getTranslator()->trans('The title is too 
                        long.', array(), 'Modules.Imageslider.Admin');
                }
                ...
                if (Tools::strlen(Tools::getValue('url_' . $language['id_lang'])) > 255) {
                    $errors[] = $this->getTranslator()->trans('The URL is 
                        too long.', array(), 'Modules.Imageslider.Admin');
                }
            ...
            if (Tools::strlen(Tools::getValue('url_' . $id_lang_default)) == 0) {
                $errors[] = $this->getTranslator()->trans('The URL is 
                    not set.', array(), 'Modules.Imageslider.Admin');
            }
            if (!Tools::isSubmit('has_picture') && (!isset($_FILES['image_' . $id_lang_default]) ||
                ...
                $errors[] = $this->getTranslator()->trans('The image 
                    is not set.', array(), 'Modules.Imageslider.Admin');
            }
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            switch ($shop_context) {
                case Shop::CONTEXT_ALL:
                    $res &= Configuration::updateValue('WBSLIDER_SPEED', (int)Tools::getValue('WBSLIDER_SPEED'));
                    $res &= Configuration::updateValue(
                        'WBSLIDER_PAUSE_ON_HOVER',
                        (int)Tools::getValue(
                            'WBSLIDER_PAUSE_ON_HOVER'
                    ...
                    $res &= Configuration::updateValue('WBSLIDER_WRAP', (int)Tools::getValue('WBSLIDER_WRAP'));
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue(
                                'WBSLIDER_SPEED',
                ...
                case Shop::CONTEXT_GROUP:
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue(
                                'WBSLIDER_SPEED',
                                (int)Tools::getValue('WBSLIDER_SPEED'),
                                false,
                                $shop_group_id
                            );
                            $res &= Configuration::updateValue(
                                'WBSLIDER_PAUSE_ON_HOVER',
                                (int)Tools::getValue('WBSLIDER_PAUSE_ON_HOVER'),
                                false,
                                $shop_group_id
                            );
                            $res &= Configuration::updateValue(
                                'WBSLIDER_WRAP',
                                (int)Tools::getValue('WBSLIDER_WRAP'),
                                false,
                                $shop_group_id
                            );
                        }
            ...
            if (!$res) {
                $errors[] = $this->displayError($this->getTranslator()->trans('The configuration 
                    could not be updated.', array(), 'Modules.Imageslider.Admin'));
            } else {
                Tools::redirectAdmin(
            ...
            $slide = new WbSlide((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new WbSlide((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    public function hookdisplayHeader($params)
    {
        $this->context->controller->registerStylesheet(
            'modules-homeslider',
            'modules/'.$this->name.'views/css/wbslider.css',
            array('media' => 'all',
            'priority' => 150)
        );
        $this->context->controller->registerJavascript(
            'modules-responsiveslides',
            'modules/'.$this->name.'views/js/responsiveslides.min.js',
            array('position' => 'bottom',
            'priority' => 150)
        );
        $this->context->controller->registerJavascript(
            'modules-homeslider',
            'modules/'.$this->name.'views/js/wbslider.js',
            array('position' => 'bottom',
            'priority' => 150)
        );
    }
    ...
    public function getWidgetVariables($hookName = null, array $configuration = null)
    {
        ...
        return array(
            'wbslider' => array(
                'speed' => $config['WBSLIDER_SPEED'],
                'pause' => $config['WBSLIDER_PAUSE_ON_HOVER'] ? 'hover' : '',
                'wrap' => $config['WBSLIDER_WRAP'] ? 'true' : 'false',
                'slides' => $slides,
            ),
        );
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute(
            'INSERT IGNORE INTO '._DB_PREFIX_.'wbslider (id_wbslider_slides, id_shop)
            SELECT id_wbslider_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'wbslider
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            ...
            FROM `'._DB_PREFIX_.'wbslider_slides` hss, `'._DB_PREFIX_.'wbslider` hs
            WHERE hss.`id_wbslider_slides` = hs.`id_wbslider_slides` 
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            'SELECT hs.`id_wbslider_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
            FROM '._DB_PREFIX_.'wbslider hs
            LEFT JOIN '._DB_PREFIX_.'wbslider_slides hss ON (hs.id_wbslider_slides = hss.id_wbslider_slides)
            LEFT JOIN '._DB_PREFIX_.'wbslider_slides_lang hssl ON (hss.id_wbslider_slides = hssl.id_wbslider_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(
                _MODULE_DIR_.'wbimageslider/views/img/'.$slide['image']
            );
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            FROM '._DB_PREFIX_.'wbslider hs
            LEFT JOIN '._DB_PREFIX_.'wbslider_slides hss ON (hs.id_wbslider_slides = hss.id_wbslider_slides)
            LEFT JOIN '._DB_PREFIX_.'wbslider_slides_lang hssl ON (hss.id_wblider_slides = hssl.id_wbslider_slides)
            WHERE hs.`id_wbslider_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_wbslider_slides` as id_slide
                FROM `'._DB_PREFIX_.'wbslider` hs
                WHERE hs.`id_wbslider_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = WbSlide::getAssociatedIdsShop((int)$slide['id_slide']);
            ...
            'form' => array(
                'legend' => array(
                    'title' => $this->getTranslator()->trans('Slide 
                        information', array(), 'Modules.Imageslider.Admin'),
                    'icon' => 'icon-cogs'
                ),
                'input' => array(
                    array(
                    ...
                    array(
                        ...
                        'label' => $this->getTranslator()->trans('Target
                         URL', array(), 'Modules.Imageslider.Admin'),
                        'name' => 'url',
                        'required' => true,
                        'lang' => true,
                    ),
                ...
                'input' => array(
                    array(
                        ...
                        'desc' => $this->getTranslator()->trans('The duration of the 
                            transition between two slides.', array(), 'Modules.Imageslider.Admin')
                    ),
                    array(
                        'type' => 'switch',
                        'label' => $this->getTranslator()->trans('Pause on 
                            hover', array(), 'Modules.Imageslider.Admin'),
                        'name' => 'WBSLIDER_PAUSE_ON_HOVER',
                        'desc' => $this->getTranslator()->trans('Stop sliding when the 
                            mouse cursor is over the slideshow.', array(), 'Modules.Imageslider.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
                    ...
                    array(
                        ...
                        'desc' => $this->getTranslator()->trans('Loop or stop after 
                            the last slide.', array(), 'Modules.Imageslider.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
    ...
    public function getConfigFieldsValues()
    {
        ...
        return array(
            ...
            'WBSLIDER_PAUSE_ON_HOVER' => Tools::getValue(
                'WBSLIDER_PAUSE_ON_HOVER',
                Configuration::get(
                    'WBSLIDER_PAUSE_ON_HOVER',
                    null,
    ...
    protected function getMultiLanguageInfoMsg()
    {
        return '<p class="alert alert-warning">'.
                    $this->getTranslator()->trans('Since multiple languages are activated on your shop, 
                        please mind to upload your image for each one of them', array(), 'Modules.Imageslider.Admin').
                '</p>';
    }
    ...
    protected function getWarningMultishopHtml()
    {
        if (Shop::getContext() == Shop::CONTEXT_GROUP || Shop::getContext() == Shop::CONTEXT_ALL) {
            ...
            $this->getTranslator()->trans('You cannot manage slides items from a "All Shops" or a "G
                roup Shop" context, select directly the shop you want to edit', array(), 'Modules.Imageslider.Admin') .
            '</p>';
        } else {
            return '';
        }
    ...
    protected function getShopContextError($shop_contextualized_name, $mode)
    {
        ...
        if ($mode == 'edit') {
            ...
            $this->trans('You can only edit this slide from the shop(s) 
                context: %s', array($shop_contextualized_name), 'Modules.Imageslider.Admin') .
            '</p>';
        } else {
            return '<p class="alert alert-danger">' .
            $this->trans('You cannot add slides from a "All Shops" or a 
                "Group Shop" context', array(), 'Modules.Imageslider.Admin') .
            '</p>';
        }
    }
        ...
        if (Shop::isFeatureActive()) {
            if (Shop::getContext() == Shop::CONTEXT_SHOP) {
                ...
                $shop_info = $this->trans('The modifications will be 
                    applied to all shops and shop groups', array(), 'Modules.Imageslider.Admin');
            }
    ...
    protected function getSharedSlideWarning()
    {
        return '<p class="alert alert-warning">'.
                    $this->trans('This slide is shared with other shops! 
                        ...
                        modifications made here', array(), 'Modules.Imageslider.Admin').
                '</p>';
    }
}
