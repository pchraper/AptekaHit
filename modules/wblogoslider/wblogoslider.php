...
include_once(_PS_MODULE_DIR_.'wblogoslider/WbHomeSlide.php');

class WbLogoSlider extends Module implements WidgetInterface
{
    protected $html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('WB Logo slider', array(
            ), 'Modules.Logoslider.Admin');
        $this->description = $this->getTranslator()->trans('Adds an image slider to your site.', array(
            ), 'Modules.Logoslider.Admin');
        $this->ps_versions_compliancy = array('min' => '1.7.1.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:wblogoslider/views/templates/hook/slider.tpl';
    ...
    public function install()
    {
        ...
        if (parent::install() &&
            ...
            $res &= Configuration::updateValue('LOGOSLIDER_SPEED', $this->default_speed);
            $res &= Configuration::updateValue('LOGOSLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover);
            $res &= Configuration::updateValue('LOGOSLIDER_WRAP', $this->default_wrap);
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 12; ++$i) {
            $slide = new WbHomeSlide();
            $slide->position = $i;
            $slide->active = 1;
            foreach ($languages as $language) {
                $slide->title[$language['id_lang']] = 'Menufacture '.$i;
                $slide->description[$language['id_lang']] = 'Test Demo';
                $slide->legend[$language['id_lang']] = 'menufacture-'.$i;
                ...
                $slide->image[$language['id_lang']] = 'menufacture-'.$i.'.jpg';
            }
            $slide->add();
        }
    }
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('LOGOSLIDER_SPEED');
            $res &= Configuration::deleteByName('LOGOSLIDER_PAUSE_ON_HOVER');
            $res &= Configuration::deleteByName('LOGOSLIDER_WRAP');
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'logoslider` (
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'logoslider_slides` (
              `id_logoslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'logoslider_slides_lang` (
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new WbHomeSlide($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'logoslider`,
            `'._DB_PREFIX_.'logoslider_slides`,
            `'._DB_PREFIX_.'logoslider_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = WbHomeSlide::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            if (!Validate::isInt(Tools::getValue('LOGOSLIDER_SPEED'))) {
                $errors[] = $this->getTranslator()->trans('Invalid values', array(), 'Modules.Logoslider.Admin');
            }
        } elseif (Tools::isSubmit('changeStatus')) {
            if (!Validate::isInt(Tools::getValue('id_slide'))) {
                $errors[] = $this->getTranslator()->trans('Invalid slide', array(), 'Modules.Logoslider.Admin');
            }
            ...
            if (!Validate::isInt(Tools::getValue('active_slide')) ||
                ...
                $errors[] = $this->getTranslator()->trans('Invalid slide
                 state.', array(), 'Modules.Logoslider.Admin');
            }
            /* Checks position */
            ...
            if (Tools::isSubmit('id_slide')) {
                if (!Validate::isInt(Tools::getValue('id_slide')) &&
                    ...
                    $errors[] = $this->getTranslator()->trans('Invalid 
                        slide ID', array(), 'Modules.Logoslider.Admin');
                }
            }
            ...
            foreach ($languages as $language) {
                if (Tools::strlen(Tools::getValue('title_' . $language['id_lang'])) > 255) {
                    $errors[] = $this->getTranslator()->trans('The title is too 
                        long.', array(), 'Modules.Logoslider.Admin');
                }
                ...
                if (Tools::strlen(Tools::getValue('url_' . $language['id_lang'])) > 255) {
                    $errors[] = $this->getTranslator()->trans('The URL is 
                        too long.', array(), 'Modules.Logoslider.Admin');
                }
            ...
            if (Tools::strlen(Tools::getValue('url_' . $id_lang_default)) == 0) {
                $errors[] = $this->getTranslator()->trans('The URL is 
                    not set.', array(), 'Modules.Logoslider.Admin');
            }
            if (!Tools::isSubmit('has_picture') && (!isset($_FILES['image_' . $id_lang_default]) ||
                ...
                $errors[] = $this->getTranslator()->trans('The image 
                    is not set.', array(), 'Modules.Logoslider.Admin');
            }
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            switch ($shop_context) {
                case Shop::CONTEXT_ALL:
                    $res &= Configuration::updateValue('LOGOSLIDER_SPEED', (int)Tools::getValue('LOGOSLIDER_SPEED'));
                    $res &= Configuration::updateValue(
                        'LOGOSLIDER_PAUSE_ON_HOVER',
                        (int)Tools::getValue(
                            'LOGOSLIDER_PAUSE_ON_HOVER'
                    ...
                    $res &= Configuration::updateValue('LOGOSLIDER_WRAP', (int)Tools::getValue('LOGOSLIDER_WRAP'));
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue(
                                'LOGOSLIDER_SPEED',
                ...
                case Shop::CONTEXT_GROUP:
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue(
                                'LOGOSLIDER_SPEED',
                                (int)Tools::getValue('LOGOSLIDER_SPEED'),
                                false,
                                $shop_group_id
                            );
                            $res &= Configuration::updateValue(
                                'LOGOSLIDER_PAUSE_ON_HOVER',
                                (int)Tools::getValue('LOGOSLIDER_PAUSE_ON_HOVER'),
                                false,
                                $shop_group_id
                            );
                            $res &= Configuration::updateValue(
                                'LOGOSLIDER_WRAP',
                                (int)Tools::getValue('LOGOSLIDER_WRAP'),
                                false,
                                $shop_group_id
                            );
                        }
            ...
            if (!$res) {
                $errors[] = $this->displayError($this->getTranslator()->trans('The configuration 
                    could not be updated.', array(), 'Modules.Logoslider.Admin'));
            } else {
                Tools::redirectAdmin(
            ...
            $slide = new WbHomeSlide((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new WbHomeSlide((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    public function getWidgetVariables($hookName = null, array $configuration = null)
    {
        ...
        return array(
            'logoslider' => array(
                'speed' => $config['LOGOSLIDER_SPEED'],
                'pause' => $config['LOGOSLIDER_PAUSE_ON_HOVER'] ? 'hover' : '',
                'wrap' => $config['LOGOSLIDER_WRAP'] ? 'true' : 'false',
                'slides' => $slides,
            ),
        );
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute(
            'INSERT IGNORE INTO '._DB_PREFIX_.'logoslider (id_logoslider_slides, id_shop)
            SELECT id_logoslider_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'logoslider
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            ...
            FROM `'._DB_PREFIX_.'logoslider_slides` hss, `'._DB_PREFIX_.'logoslider` hs
            WHERE hss.`id_logoslider_slides` = hs.`id_logoslider_slides` 
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            'SELECT hs.`id_logoslider_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
            FROM '._DB_PREFIX_.'logoslider hs
            LEFT JOIN '._DB_PREFIX_.'logoslider_slides hss ON (hs.id_logoslider_slides = hss.id_logoslider_slides)
            LEFT JOIN '._DB_PREFIX_.'logoslider_slides_lang hssl
            ON (hss.id_logoslider_slides = hssl.id_logoslider_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(
                _MODULE_DIR_.'wblogoslider/views/img/'.$slide['image']
            );
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            FROM '._DB_PREFIX_.'logoslider hs
            LEFT JOIN '._DB_PREFIX_.'logoslider_slides hss ON (hs.id_logoslider_slides = hss.id_logoslider_slides)
            LEFT JOIN '._DB_PREFIX_.'logoslider_slides_lang hssl ON (hss.id_wblider_slides = hssl.id_logoslider_slides)
            WHERE hs.`id_logoslider_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_logoslider_slides` as id_slide
                FROM `'._DB_PREFIX_.'logoslider` hs
                WHERE hs.`id_logoslider_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = WbHomeSlide::getAssociatedIdsShop((int)$slide['id_slide']);
            ...
            'form' => array(
                'legend' => array(
                    'title' => $this->getTranslator()->trans('Slide 
                        information', array(), 'Modules.Logoslider.Admin'),
                    'icon' => 'icon-cogs'
                ),
                'input' => array(
                    array(
                    ...
                    array(
                        ...
                        'label' => $this->getTranslator()->trans('Target
                         URL', array(), 'Modules.Logoslider.Admin'),
                        'name' => 'url',
                        'required' => true,
                        'lang' => true,
                    ),
                    array(
                        'type' => 'text',
                        'label' => $this->getTranslator()->trans('Caption', array(), 'Modules.Logoslider.Admin'),
                        'name' => 'legend',
                        'lang' => true,
                    ),
                    array(
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    array(
                        'type' => 'text',
                        'label' => $this->getTranslator()->trans('Speed', array(), 'Modules.Logoslider.Admin'),
                        'name' => 'LOGOSLIDER_SPEED',
                        'suffix' => 'milliseconds',
                        'class' => 'fixed-width-sm',
                        'desc' => $this->getTranslator()->trans('The duration of the 
                            transition between two slides.', array(), 'Modules.Logoslider.Admin')
                    ),
                    array(
                        'type' => 'switch',
                        'label' => $this->getTranslator()->trans('Pause on 
                            hover', array(), 'Modules.Logoslider.Admin'),
                        'name' => 'LOGOSLIDER_PAUSE_ON_HOVER',
                        'desc' => $this->getTranslator()->trans('Stop sliding when the 
                            mouse cursor is over the slideshow.', array(), 'Modules.Logoslider.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
                    ...
                    array(
                        ...
                        'name' => 'LOGOSLIDER_WRAP',
                        'desc' => $this->getTranslator()->trans('Loop or stop after 
                            the last slide.', array(), 'Modules.Logoslider.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
    ...
    public function getConfigFieldsValues()
    {
        ...
        return array(
            'LOGOSLIDER_SPEED' => Tools::getValue(
                'LOGOSLIDER_SPEED',
                Configuration::get(
                    'LOGOSLIDER_SPEED',
                    null,
            ...
            'LOGOSLIDER_PAUSE_ON_HOVER' => Tools::getValue(
                'LOGOSLIDER_PAUSE_ON_HOVER',
                Configuration::get(
                    'LOGOSLIDER_PAUSE_ON_HOVER',
                    null,
            ...
            'LOGOSLIDER_WRAP' => Tools::getValue(
                'LOGOSLIDER_WRAP',
                Configuration::get(
                    'LOGOSLIDER_WRAP',
                    null,
    ...
    protected function getMultiLanguageInfoMsg()
    {
        return '<p class="alert alert-warning">'.
                    $this->getTranslator()->trans('Since multiple languages are activated on your shop, 
                        please mind to upload your image for each one of them', array(), 'Modules.Logoslider.Admin').
                '</p>';
    }
    ...
    protected function getWarningMultishopHtml()
    {
        if (Shop::getContext() == Shop::CONTEXT_GROUP || Shop::getContext() == Shop::CONTEXT_ALL) {
            ...
            $this->getTranslator()->trans('You cannot manage slides items from a "All Shops" or a "G
                roup Shop" context, select directly the shop you want to edit', array(), 'Modules.Logoslider.Admin') .
            '</p>';
        } else {
            return '';
        }
    ...
    protected function getShopContextError($shop_contextualized_name, $mode)
    {
        ...
        if ($mode == 'edit') {
            ...
            $this->trans('You can only edit this slide from the shop(s) 
                context: %s', array($shop_contextualized_name), 'Modules.Logoslider.Admin') .
            '</p>';
        } else {
            return '<p class="alert alert-danger">' .
            $this->trans('You cannot add slides from a "All Shops" or a 
                "Group Shop" context', array(), 'Modules.Logoslider.Admin') .
            '</p>';
        }
    }
        ...
        if (Shop::isFeatureActive()) {
            if (Shop::getContext() == Shop::CONTEXT_SHOP) {
                ...
                $shop_info = $this->trans('The modifications will be 
                    applied to all shops and shop groups', array(), 'Modules.Logoslider.Admin');
            }
    ...
    protected function getSharedSlideWarning()
    {
        return '<p class="alert alert-warning">'.
                    $this->trans('This slide is shared with other shops! 
                        ...
                        modifications made here', array(), 'Modules.Logoslider.Admin').
                '</p>';
    }
}
