...
class WbThemeCustomizer extends Module
{
    ...
    private function createConfigs()
    {
        ...
        $response &= Configuration::updateValue('WB_darklightLayout', 'lightmode');

        $response = Configuration::updateValue('WB_showPanelTool', 1);

        // Header Options
        $response &= Configuration::updateValue('WB_showDarkLightMenu', 1);
    ...
    private function deleteConfigs()
    {
        ...
        $response &= Configuration::deleteByName('WB_darklightLayout');

        // Header Options
        $response &= Configuration::deleteByName('WB_showDarkLightMenu');
    ...
    private function defineArrays()
    {
        ...
        $this->cssRules = array(
            ...
            'WB_mainColorScheme' => array(
                ...
                array(
                    'selector' => '.wb-menu-vertical ul li.level-1:hover > a, .view_more a:hover span,.des_namepost,.foot-copy ._blank:hover,.footer-container li a:hover, #footer .lnk_wishlist:hover, .foot-payment i:hover,a:hover, a:focus,.read_more:hover, .read_more:focus,.cart-name:hover, .thumbnail-container .product-title:hover, .thumbnail-container .product-title a:hover,.wbpc-main .count,.next-prevb .owl-theme .owl-nav [class*="owl-"]:hover,.product-flag,.first-word span,.statmenu a:hover,#_desktop_language_selector button:hover, #_desktop_currency_selector button:hover, .hcom:hover a, .wishl:hover a, .usvg:hover, .hcart:hover',
                    'rule' => 'color'
                ),

                array(
                    'selector' => '.categorybr:hover,.deliveryinfo .ses:hover ul li.ser-svg,#search_widget',
                    'rule' => 'border-color'
                ),

                array(
                    'selector' => '.read_more:hover svg,#footer_contact .icon svg,#_desktop_language_selector button:hover svg, #_desktop_currency_selector button:hover svg, .hcom:hover svg, .wishl:hover svg, .hcart:hover svg, .user-info:hover svg',
                    'rule' => 'fill'
                )
            ),
            ...
            'WB_activeColorScheme' => array(
                ...
                array(
                    'selector' => '.header-nav .dropdown-menu, .user-down, .language-selector .dropdown-menu, .currency-selector .dropdown-menu,.head-cart-drop,.language-selector .dropdown-menu, .currency-selector .dropdown-menu,.wb-menu-vertical .menu-dropdown,.view_menu .more-menu',
                ...
                array(
                    'selector' => '.test-img svg,.headsvg svg',
                    'rule' => 'fill'
                )
            ),
        );
    ...
    private function _displayForm()
    {
        ...
        $fields_form = array(
            'wb-general' => array(
                'form' => array(
                    ...
                    'input' => array(
                        ...
                        array(
                            'type' => 'select',
                            'name' => 'WB_darklightLayout',
                            'label' => $this->l('Dark and Light Mode'),
                            'required' => false,
                            'lang' => false,
                            'options' => array(
                                'query' => $darklightTypes,
                                'id' => 'value',
                                'name' => 'name'
                            )
                        )
            ...
            'wb-header' => array(
                'form' => array(
                    'legend' => array(
                        'title' => $this->l('User Dark/Light'),
                        'icon' => 'icon-cog'
                    ),
                    'input' => array(
                        array(
                            'type' => 'switch',
                            'label' => $this->l('Show Dark/Light Mode'),
                            'name' => 'WB_showDarkLightMenu',
                            'required' => false,
                            'is_bool' => true,
                            'values' => array(
                                array(
    ...
    private function _getTabs()
    {
        $tabArray = array(
            ...
            'Visible in Live Dark/Light Mode' => 'fieldset_wb-header_1',
    ...
    private function prepHook($params)
    {
        ...
        $WB_showDarkLightMenu = Configuration::get('WB_showDarkLightMenu');
        if ($WB_showDarkLightMenu) {
            $this->context->controller->addJS($this->_path.'views/js/front/colorpicker.js');
            $this->context->controller->addJS($this->_path.'views/js/front/jquery.wbcolortool.js');
            $this->context->controller->addCSS(_MODULE_DIR_ . $this->name . '/views/css/front/wb.cltool.css');
            $this->smarty->assignGlobal(
                'WB_DARKLIGHTPANELTOOL_TPL',
                _PS_MODULE_DIR_.$this->name.'/views/templates/front/wbdarklight.tpl'
            );
        }
        $this->context->controller->addCSS(_MODULE_DIR_ . $this->name . '/views/css/front/wb.cltool.css');
