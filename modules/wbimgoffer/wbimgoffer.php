...
include_once(_PS_MODULE_DIR_.'wbimgoffer/WbHomeOffer.php');

class WbImgOffer extends Module implements WidgetInterface
{
    protected $html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('WB Offer banner', array(
            ), 'Modules.Imgoffer.Admin');
        $this->description = $this->getTranslator()->trans('Adds an offer banner to your site.', array(
            ), 'Modules.Imgoffer.Admin');
        $this->ps_versions_compliancy = array('min' => '1.7.1.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:wbimgoffer/views/templates/hook/slider.tpl';
    ...
    public function install()
    {
        ...
        if (parent::install() &&
            ...
            $res &= Configuration::updateValue('IMGOFFER_SPEED', $this->default_speed);
            $res &= Configuration::updateValue('IMGOFFER_PAUSE_ON_HOVER', $this->default_pause_on_hover);
            $res &= Configuration::updateValue('IMGOFFER_WRAP', $this->default_wrap);
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 1; ++$i) {
            $slide = new WbHomeOffer();
            $slide->position = $i;
            $slide->active = 1;
            foreach ($languages as $language) {
                $slide->title[$language['id_lang']] = 'Offerbanner '.$i;
                $slide->description[$language['id_lang']] = '<h2>Cloths Collection For Women 2019</h2><p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et..</p>';
                $slide->legend[$language['id_lang']] = 'offerbanner-'.$i;
                $slide->url[$language['id_lang']] = 'http://www.prestashop.com/?utm_source=back-office&utm_medium=v17_imgoffer'
                ...
                $slide->image[$language['id_lang']] = 'offerbanner-'.$i.'.jpg';
            }
            $slide->add();
        }
    }
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('IMGOFFER_SPEED');
            $res &= Configuration::deleteByName('IMGOFFER_PAUSE_ON_HOVER');
            $res &= Configuration::deleteByName('IMGOFFER_WRAP');
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'imgoffer` (
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'imgoffer_slides` (
              `id_imgoffer_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'imgoffer_slides_lang` (
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new WbHomeOffer($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'imgoffer`,
            `'._DB_PREFIX_.'imgoffer_slides`,
            `'._DB_PREFIX_.'imgoffer_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = WbHomeOffer::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            if (!Validate::isInt(Tools::getValue('IMGOFFER_SPEED'))) {
                $errors[] = $this->getTranslator()->trans('Invalid values', array(), 'Modules.Imgoffer.Admin');
            }
        } elseif (Tools::isSubmit('changeStatus')) {
            if (!Validate::isInt(Tools::getValue('id_slide'))) {
                $errors[] = $this->getTranslator()->trans('Invalid slide', array(), 'Modules.Imgoffer.Admin');
            }
            ...
            if (!Validate::isInt(Tools::getValue('active_slide')) ||
                ...
                $errors[] = $this->getTranslator()->trans('Invalid slide
                 state.', array(), 'Modules.Imgoffer.Admin');
            }
            /* Checks position */
            ...
            if (Tools::isSubmit('id_slide')) {
                if (!Validate::isInt(Tools::getValue('id_slide')) &&
                    ...
                    $errors[] = $this->getTranslator()->trans('Invalid 
                        slide ID', array(), 'Modules.Imgoffer.Admin');
                }
            }
            ...
            foreach ($languages as $language) {
                if (Tools::strlen(Tools::getValue('title_' . $language['id_lang'])) > 255) {
                    $errors[] = $this->getTranslator()->trans('The title is too 
                        long.', array(), 'Modules.Imgoffer.Admin');
                }
                ...
                if (Tools::strlen(Tools::getValue('url_' . $language['id_lang'])) > 255) {
                    $errors[] = $this->getTranslator()->trans('The URL is 
                        too long.', array(), 'Modules.Imgoffer.Admin');
                }
            ...
            if (Tools::strlen(Tools::getValue('url_' . $id_lang_default)) == 0) {
                $errors[] = $this->getTranslator()->trans('The URL is 
                    not set.', array(), 'Modules.Imgoffer.Admin');
            }
            if (!Tools::isSubmit('has_picture') && (!isset($_FILES['image_' . $id_lang_default]) ||
                ...
                $errors[] = $this->getTranslator()->trans('The image 
                    is not set.', array(), 'Modules.Imgoffer.Admin');
            }
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            switch ($shop_context) {
                case Shop::CONTEXT_ALL:
                    $res &= Configuration::updateValue('IMGOFFER_SPEED', (int)Tools::getValue('IMGOFFER_SPEED'));
                    $res &= Configuration::updateValue(
                        'IMGOFFER_PAUSE_ON_HOVER',
                        (int)Tools::getValue(
                            'IMGOFFER_PAUSE_ON_HOVER'
                    ...
                    $res &= Configuration::updateValue('IMGOFFER_WRAP', (int)Tools::getValue('IMGOFFER_WRAP'));
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue(
                                'IMGOFFER_SPEED',
                ...
                case Shop::CONTEXT_GROUP:
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue(
                                'IMGOFFER_SPEED',
                                (int)Tools::getValue('IMGOFFER_SPEED'),
                                false,
                                $shop_group_id
                            );
                            $res &= Configuration::updateValue(
                                'IMGOFFER_PAUSE_ON_HOVER',
                                (int)Tools::getValue('IMGOFFER_PAUSE_ON_HOVER'),
                                false,
                                $shop_group_id
                            );
                            $res &= Configuration::updateValue(
                                'IMGOFFER_WRAP',
                                (int)Tools::getValue('IMGOFFER_WRAP'),
                                false,
                                $shop_group_id
                            );
                        }
            ...
            if (!$res) {
                $errors[] = $this->displayError($this->getTranslator()->trans('The configuration 
                    could not be updated.', array(), 'Modules.Imgoffer.Admin'));
            } else {
                Tools::redirectAdmin(
            ...
            $slide = new WbHomeOffer((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new WbHomeOffer((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    public function getWidgetVariables($hookName = null, array $configuration = null)
    {
        ...
        return array(
            'imgoffer' => array(
                'speed' => $config['IMGOFFER_SPEED'],
                'pause' => $config['IMGOFFER_PAUSE_ON_HOVER'] ? 'hover' : '',
                'wrap' => $config['IMGOFFER_WRAP'] ? 'true' : 'false',
                'slides' => $slides,
            ),
        );
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute(
            'INSERT IGNORE INTO '._DB_PREFIX_.'imgoffer (id_imgoffer_slides, id_shop)
            SELECT id_imgoffer_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'imgoffer
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            ...
            FROM `'._DB_PREFIX_.'imgoffer_slides` hss, `'._DB_PREFIX_.'imgoffer` hs
            WHERE hss.`id_imgoffer_slides` = hs.`id_imgoffer_slides` 
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            'SELECT hs.`id_imgoffer_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
            FROM '._DB_PREFIX_.'imgoffer hs
            LEFT JOIN '._DB_PREFIX_.'imgoffer_slides hss ON (hs.id_imgoffer_slides = hss.id_imgoffer_slides)
            LEFT JOIN '._DB_PREFIX_.'imgoffer_slides_lang hssl
            ON (hss.id_imgoffer_slides = hssl.id_imgoffer_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(
                _MODULE_DIR_.'wbimgoffer/views/img/'.$slide['image']
            );
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            FROM '._DB_PREFIX_.'imgoffer hs
            LEFT JOIN '._DB_PREFIX_.'imgoffer_slides hss ON (hs.id_imgoffer_slides = hss.id_imgoffer_slides)
            LEFT JOIN '._DB_PREFIX_.'imgoffer_slides_lang hssl ON (hss.id_wblider_slides = hssl.id_imgoffer_slides)
            WHERE hs.`id_imgoffer_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_imgoffer_slides` as id_slide
                FROM `'._DB_PREFIX_.'imgoffer` hs
                WHERE hs.`id_imgoffer_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = WbHomeOffer::getAssociatedIdsShop((int)$slide['id_slide']);
            ...
            'form' => array(
                'legend' => array(
                    'title' => $this->getTranslator()->trans('Slide 
                        information', array(), 'Modules.Imgoffer.Admin'),
                    'icon' => 'icon-cogs'
                ),
                'input' => array(
                    array(
                    ...
                    array(
                        ...
                        'label' => $this->getTranslator()->trans('Target
                         URL', array(), 'Modules.Imgoffer.Admin'),
                        'name' => 'url',
                        'required' => true,
                        'lang' => true,
                    ),
                    array(
                        'type' => 'text',
                        'label' => $this->getTranslator()->trans('Caption', array(), 'Modules.Imgoffer.Admin'),
                        'name' => 'legend',
                        'lang' => true,
                    ),
                    array(
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    array(
                        'type' => 'text',
                        'label' => $this->getTranslator()->trans('Speed', array(), 'Modules.Imgoffer.Admin'),
                        'name' => 'IMGOFFER_SPEED',
                        'suffix' => 'milliseconds',
                        'class' => 'fixed-width-sm',
                        'desc' => $this->getTranslator()->trans('The duration of the 
                            transition between two slides.', array(), 'Modules.Imgoffer.Admin')
                    ),
                    array(
                        'type' => 'switch',
                        'label' => $this->getTranslator()->trans('Pause on 
                            hover', array(), 'Modules.Imgoffer.Admin'),
                        'name' => 'IMGOFFER_PAUSE_ON_HOVER',
                        'desc' => $this->getTranslator()->trans('Stop sliding when the 
                            mouse cursor is over the slideshow.', array(), 'Modules.Imgoffer.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
                    ...
                    array(
                        ...
                        'desc' => $this->getTranslator()->trans('Loop or stop after 
                            the last slide.', array(), 'Modules.Imgoffer.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
    ...
    public function getConfigFieldsValues()
    {
        ...
        return array(
            ...
            'IMGOFFER_PAUSE_ON_HOVER' => Tools::getValue(
                'IMGOFFER_PAUSE_ON_HOVER',
                Configuration::get(
                    'IMGOFFER_PAUSE_ON_HOVER',
                    null,
    ...
    protected function getMultiLanguageInfoMsg()
    {
        return '<p class="alert alert-warning">'.
                    $this->getTranslator()->trans('Since multiple languages are activated on your shop, 
                        please mind to upload your image for each one of them', array(), 'Modules.Imgoffer.Admin').
                '</p>';
    }
    ...
    protected function getWarningMultishopHtml()
    {
        if (Shop::getContext() == Shop::CONTEXT_GROUP || Shop::getContext() == Shop::CONTEXT_ALL) {
            ...
            $this->getTranslator()->trans('You cannot manage slides items from a "All Shops" or a "G
                roup Shop" context, select directly the shop you want to edit', array(), 'Modules.Imgoffer.Admin') .
            '</p>';
        } else {
            return '';
        }
    ...
    protected function getShopContextError($shop_contextualized_name, $mode)
    {
        ...
        if ($mode == 'edit') {
            ...
            $this->trans('You can only edit this slide from the shop(s) 
                context: %s', array($shop_contextualized_name), 'Modules.Imgoffer.Admin') .
            '</p>';
        } else {
            return '<p class="alert alert-danger">' .
            $this->trans('You cannot add slides from a "All Shops" or a 
                "Group Shop" context', array(), 'Modules.Imgoffer.Admin') .
            '</p>';
        }
    }
        ...
        if (Shop::isFeatureActive()) {
            if (Shop::getContext() == Shop::CONTEXT_SHOP) {
                ...
                $shop_info = $this->trans('The modifications will be 
                    applied to all shops and shop groups', array(), 'Modules.Imgoffer.Admin');
            }
    ...
    protected function getSharedSlideWarning()
    {
        return '<p class="alert alert-warning">'.
                    $this->trans('This slide is shared with other shops! 
                        ...
                        modifications made here', array(), 'Modules.Imgoffer.Admin').
                '</p>';
    }
}
