...
class BlockWishList extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->description = $this->l('Adds a block containing the customer wishlists.');
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('viewblockwishlist') && $id = Tools::getValue('id_product')) {
            ...
            if ($activated != 0 && $activated != 1) {
                $this->html .= '<div class="alert error alert-danger">'.$this->l('
                    Activate module : Invalid choice.').'</div>';
            }
    ...
    public function hookdisplayTop()
    {
        ...
        $useSSL = ((isset($this->ssl) && $this->ssl && Configuration::
            get('PS_SSL_ENABLED')) || Tools::usingSecureMode()) ? true : false;
        ...
        if ($this->context->customer->isLogged()) {
            ...
            $this->smarty->assign(
                array(
                    ...
                    'isLogged' => true,
                    'wishlist_products' => ($id_wishlist == false ? false : WishList::
        ...
        $this->context->smarty->assign(
            array(
                ...
                'count_product' => (int)Db::getInstance()->getValue('
                    SELECT count(id_wishlist_product) FROM '._DB_PREFIX_.'wishlist w, '.
                    _DB_PREFIX_.'wishlist_product wp where w.id_wishlist = wp.id_wishlist
    ...
    {
        $this->context->controller->addCSS(($this->_path).'/views/css/blockwishlist.css', 'all');
        $this->context->controller->addJS(($this->_path).'/views/js/ajax-wishlist.js');

        $this->smarty->assign(array('wishlist_link' => $this->context->link->
            getModuleLink('blockwishlist', 'mywishlist')));
    }
    ...
    private function _displayProducts($id_wishlist)
    {
        ...
        $products = WishList::getProductByIdCustomer($id_wishlist, $wishlist->id_customer, $this->context
            ->language->id);
    ...
    public function hookAdminCustomers($params)
    {
        ...
        if (!count($wishlists)) {
            ...
            $this->html .= '<span>'.$this->l('Wishlist').': </span>
    ...
    */
    ...
    public function renderJS()
    {
        return "<script>
            $(document).ready(function () {
             $('#id_customer, #id_wishlist').change( function () { $('#module_form').submit();}); });
        </script>";
    }
    ...
    public function renderForm()
    {
        ...
        $helper->allow_employee_form_lang = Configuration::
        get('PS_BO_ALLOW_EMPLOYEE_FORM_LANG') ? Configuration::get('PS_BO_ALLOW_EMPLOYEE_FORM_LANG') : 0;
    ...
    public function renderList($id_wishlist)
    {
        ...
        foreach ($products as $key => $val) {
            ...
            $products[$key]['image'] = $this->context->link->
            getImageLink($val['link_rewrite'], $image['id_image'], ImageType::getFormatedName('small'));
        }
