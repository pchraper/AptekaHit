...
include_once(_PS_MODULE_DIR_.'wbimgbanner/WbHomeBanner.php');

class WbImgBanner extends Module implements WidgetInterface
{
    protected $html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('WB image banner', array(
            ), 'Modules.Imgbanner.Admin');
        $this->description = $this->getTranslator()->trans('Adds an image banner to your site.', array(
            ), 'Modules.Imgbanner.Admin');
        $this->ps_versions_compliancy = array('min' => '1.7.1.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:wbimgbanner/views/templates/hook/slider.tpl';
    ...
    public function install()
    {
        ...
        if (parent::install() &&
            ...
            $res &= Configuration::updateValue('IMGBANNER_SPEED', $this->default_speed);
            $res &= Configuration::updateValue('IMGBANNER_PAUSE_ON_HOVER', $this->default_pause_on_hover);
            $res &= Configuration::updateValue('IMGBANNER_WRAP', $this->default_wrap);
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 3; ++$i) {
            $slide = new WbHomeBanner();
            $slide->position = $i;
            $slide->active = 1;
            foreach ($languages as $language) {
                $slide->title[$language['id_lang']] = 'banner '.$i;
                $slide->description[$language['id_lang']] = '<div class="ser-img"></div>
<h4>free shipping</h4>
<p>Hourly time slots deliveries.</p>';
                $slide->legend[$language['id_lang']] = 'banner-'.$i;
                $slide->url[$language['id_lang']] = 'http://www.prestashop.com/?utm_source=back-office&utm_medium=v17_imgbanner'
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('IMGBANNER_SPEED');
            $res &= Configuration::deleteByName('IMGBANNER_PAUSE_ON_HOVER');
            $res &= Configuration::deleteByName('IMGBANNER_WRAP');
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'imgbanner` (
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'imgbanner_slides` (
              `id_imgbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'imgbanner_slides_lang` (
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new WbHomeBanner($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'imgbanner`,
            `'._DB_PREFIX_.'imgbanner_slides`,
            `'._DB_PREFIX_.'imgbanner_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = WbHomeBanner::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            if (!Validate::isInt(Tools::getValue('IMGBANNER_SPEED'))) {
                $errors[] = $this->getTranslator()->trans('Invalid values', array(), 'Modules.Imgbanner.Admin');
            }
        } elseif (Tools::isSubmit('changeStatus')) {
            if (!Validate::isInt(Tools::getValue('id_slide'))) {
                $errors[] = $this->getTranslator()->trans('Invalid slide', array(), 'Modules.Imgbanner.Admin');
            }
            ...
            if (!Validate::isInt(Tools::getValue('active_slide')) ||
                ...
                $errors[] = $this->getTranslator()->trans('Invalid slide
                 state.', array(), 'Modules.Imgbanner.Admin');
            }
            /* Checks position */
            ...
            if (Tools::isSubmit('id_slide')) {
                if (!Validate::isInt(Tools::getValue('id_slide')) &&
                    ...
                    $errors[] = $this->getTranslator()->trans('Invalid 
                        slide ID', array(), 'Modules.Imgbanner.Admin');
                }
            }
            ...
            foreach ($languages as $language) {
                if (Tools::strlen(Tools::getValue('title_' . $language['id_lang'])) > 255) {
                    $errors[] = $this->getTranslator()->trans('The title is too 
                        long.', array(), 'Modules.Imgbanner.Admin');
                }
                ...
                if (Tools::strlen(Tools::getValue('url_' . $language['id_lang'])) > 255) {
                    $errors[] = $this->getTranslator()->trans('The URL is 
                        too long.', array(), 'Modules.Imgbanner.Admin');
                }
            ...
            if (Tools::strlen(Tools::getValue('url_' . $id_lang_default)) == 0) {
                $errors[] = $this->getTranslator()->trans('The URL is 
                    not set.', array(), 'Modules.Imgbanner.Admin');
            }
            if (!Tools::isSubmit('has_picture') && (!isset($_FILES['image_' . $id_lang_default]) ||
                ...
                $errors[] = $this->getTranslator()->trans('The image 
                    is not set.', array(), 'Modules.Imgbanner.Admin');
            }
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            switch ($shop_context) {
                case Shop::CONTEXT_ALL:
                    $res &= Configuration::updateValue('IMGBANNER_SPEED', (int)Tools::getValue('IMGBANNER_SPEED'));
                    $res &= Configuration::updateValue(
                        'IMGBANNER_PAUSE_ON_HOVER',
                        (int)Tools::getValue(
                            'IMGBANNER_PAUSE_ON_HOVER'
                    ...
                    $res &= Configuration::updateValue('IMGBANNER_WRAP', (int)Tools::getValue('IMGBANNER_WRAP'));
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue(
                                'IMGBANNER_SPEED',
                ...
                case Shop::CONTEXT_GROUP:
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue(
                                'IMGBANNER_SPEED',
                                (int)Tools::getValue('IMGBANNER_SPEED'),
                                false,
                                $shop_group_id
                            );
                            $res &= Configuration::updateValue(
                                'IMGBANNER_PAUSE_ON_HOVER',
                                (int)Tools::getValue('IMGBANNER_PAUSE_ON_HOVER'),
                                false,
                                $shop_group_id
                            );
                            $res &= Configuration::updateValue(
                                'IMGBANNER_WRAP',
                                (int)Tools::getValue('IMGBANNER_WRAP'),
                                false,
                                $shop_group_id
                            );
                        }
            ...
            if (!$res) {
                $errors[] = $this->displayError($this->getTranslator()->trans('The configuration 
                    could not be updated.', array(), 'Modules.Imgbanner.Admin'));
            } else {
                Tools::redirectAdmin(
            ...
            $slide = new WbHomeBanner((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new WbHomeBanner((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    public function getWidgetVariables($hookName = null, array $configuration = null)
    {
        ...
        return array(
            'imgbanner' => array(
                'speed' => $config['IMGBANNER_SPEED'],
                'pause' => $config['IMGBANNER_PAUSE_ON_HOVER'] ? 'hover' : '',
                'wrap' => $config['IMGBANNER_WRAP'] ? 'true' : 'false',
                'slides' => $slides,
            ),
        );
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute(
            'INSERT IGNORE INTO '._DB_PREFIX_.'imgbanner (id_imgbanner_slides, id_shop)
            SELECT id_imgbanner_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'imgbanner
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            ...
            FROM `'._DB_PREFIX_.'imgbanner_slides` hss, `'._DB_PREFIX_.'imgbanner` hs
            WHERE hss.`id_imgbanner_slides` = hs.`id_imgbanner_slides` 
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            'SELECT hs.`id_imgbanner_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
            FROM '._DB_PREFIX_.'imgbanner hs
            LEFT JOIN '._DB_PREFIX_.'imgbanner_slides hss ON (hs.id_imgbanner_slides = hss.id_imgbanner_slides)
            LEFT JOIN '._DB_PREFIX_.'imgbanner_slides_lang hssl
            ON (hss.id_imgbanner_slides = hssl.id_imgbanner_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(
                _MODULE_DIR_.'wbimgbanner/views/img/'.$slide['image']
            );
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            FROM '._DB_PREFIX_.'imgbanner hs
            LEFT JOIN '._DB_PREFIX_.'imgbanner_slides hss ON (hs.id_imgbanner_slides = hss.id_imgbanner_slides)
            LEFT JOIN '._DB_PREFIX_.'imgbanner_slides_lang hssl ON (hss.id_imgbanner_slides = hssl.id_imgbanner_slides)
            WHERE hs.`id_imgbanner_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_imgbanner_slides` as id_slide
                FROM `'._DB_PREFIX_.'imgbanner` hs
                WHERE hs.`id_imgbanner_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = WbHomeBanner::getAssociatedIdsShop((int)$slide['id_slide']);
            ...
            'form' => array(
                'legend' => array(
                    'title' => $this->getTranslator()->trans('Slide 
                        information', array(), 'Modules.Imgbanner.Admin'),
                    'icon' => 'icon-cogs'
                ),
                'input' => array(
                    array(
                    ...
                    array(
                        ...
                        'label' => $this->getTranslator()->trans('Target
                         URL', array(), 'Modules.Imgbanner.Admin'),
                        'name' => 'url',
                        'required' => true,
                        'lang' => true,
                    ),
                    array(
                        'type' => 'text',
                        'label' => $this->getTranslator()->trans('Caption', array(), 'Modules.Imgbanner.Admin'),
                        'name' => 'legend',
                        'lang' => true,
                    ),
                    array(
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    array(
                        'type' => 'text',
                        'label' => $this->getTranslator()->trans('Speed', array(), 'Modules.Imgbanner.Admin'),
                        'name' => 'IMGBANNER_SPEED',
                        'suffix' => 'milliseconds',
                        'class' => 'fixed-width-sm',
                        'desc' => $this->getTranslator()->trans('The duration of the 
                            transition between two slides.', array(), 'Modules.Imgbanner.Admin')
                    ),
                    array(
                        'type' => 'switch',
                        'label' => $this->getTranslator()->trans('Pause on 
                            hover', array(), 'Modules.Imgbanner.Admin'),
                        'name' => 'IMGBANNER_PAUSE_ON_HOVER',
                        'desc' => $this->getTranslator()->trans('Stop sliding when the 
                            mouse cursor is over the slideshow.', array(), 'Modules.Imgbanner.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
                    ...
                    array(
                        ...
                        'desc' => $this->getTranslator()->trans('Loop or stop after 
                            the last slide.', array(), 'Modules.Imgbanner.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
    ...
    public function getConfigFieldsValues()
    {
        ...
        return array(
            'IMGBANNER_SPEED' => Tools::getValue(
                'IMGBANNER_SPEED',
                Configuration::get(
                    'IMGBANNER_SPEED',
                    null,
            ...
            'IMGBANNER_PAUSE_ON_HOVER' => Tools::getValue(
                'IMGBANNER_PAUSE_ON_HOVER',
                Configuration::get(
                    'IMGBANNER_PAUSE_ON_HOVER',
                    null,
    ...
    protected function getMultiLanguageInfoMsg()
    {
        return '<p class="alert alert-warning">'.
                    $this->getTranslator()->trans('Since multiple languages are activated on your shop, 
                        please mind to upload your image for each one of them', array(), 'Modules.Imgbanner.Admin').
                '</p>';
    }
    ...
    protected function getWarningMultishopHtml()
    {
        if (Shop::getContext() == Shop::CONTEXT_GROUP || Shop::getContext() == Shop::CONTEXT_ALL) {
            ...
            $this->getTranslator()->trans('You cannot manage slides items from a "All Shops" or a "G
                roup Shop" context, select directly the shop you want to edit', array(), 'Modules.Imgbanner.Admin') .
            '</p>';
        } else {
            return '';
        }
    ...
    protected function getShopContextError($shop_contextualized_name, $mode)
    {
        ...
        if ($mode == 'edit') {
            ...
            $this->trans('You can only edit this slide from the shop(s) 
                context: %s', array($shop_contextualized_name), 'Modules.Imgbanner.Admin') .
            '</p>';
        } else {
            return '<p class="alert alert-danger">' .
            $this->trans('You cannot add slides from a "All Shops" or a 
                "Group Shop" context', array(), 'Modules.Imgbanner.Admin') .
            '</p>';
        }
    }
        ...
        if (Shop::isFeatureActive()) {
            if (Shop::getContext() == Shop::CONTEXT_SHOP) {
                ...
                $shop_info = $this->trans('The modifications will be 
                    applied to all shops and shop groups', array(), 'Modules.Imgbanner.Admin');
            }
    ...
    protected function getSharedSlideWarning()
    {
        return '<p class="alert alert-warning">'.
                    $this->trans('This slide is shared with other shops! 
                        ...
                        modifications made here', array(), 'Modules.Imgbanner.Admin').
                '</p>';
    }
}
