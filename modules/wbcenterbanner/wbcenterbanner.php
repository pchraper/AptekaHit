...
include_once(_PS_MODULE_DIR_.'wbcenterbanner/WbBanner.php');

class WbCenterBanner extends Module implements WidgetInterface
{
    protected $html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('WB Custom Image banner', array(
            ), 'Modules.Centerbanner.Admin');
        $this->description = $this->getTranslator()->trans('Adds a banner image to your site.', array(
            ), 'Modules.Centerbanner.Admin');
        $this->ps_versions_compliancy = array('min' => '1.7.1.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:wbcenterbanner/views/templates/hook/slider.tpl';
    ...
    public function install()
    {
        ...
        if (parent::install() &&
            ...
            $res &= Configuration::updateValue('WBBANNER_SPEED', $this->default_speed);
            $res &= Configuration::updateValue('WBBANNER_PAUSE_ON_HOVER', $this->default_pause_on_hover);
            $res &= Configuration::updateValue('WBBANNER_WRAP', $this->default_wrap);
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 2; ++$i) {
            ...
            foreach ($languages as $language) {
                $slide->title[$language['id_lang']] = 'CBanner '.$i;
                ...
                $slide->legend[$language['id_lang']] = 'cbanner-'.$i;
                $slide->url[$language['id_lang']] = 'http://www.prestashop.com/?utm_source=back-office&utm_medium=v17_wbbanner'
                ...
                $slide->image[$language['id_lang']] = 'cbanner-'.$i.'.jpg';
            }
            $slide->add();
        }
    }
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('WBBANNER_SPEED');
            $res &= Configuration::deleteByName('WBBANNER_PAUSE_ON_HOVER');
            $res &= Configuration::deleteByName('WBBANNER_WRAP');
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wbbanner` (
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wbbanner_slides` (
              `id_wbbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wbbanner_slides_lang` (
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new WbBanner($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'wbbanner`,
            `'._DB_PREFIX_.'wbbanner_slides`,
            `'._DB_PREFIX_.'wbbanner_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = WbBanner::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            if (!Validate::isInt(Tools::getValue('WBBANNER_SPEED'))) {
                $errors[] = $this->getTranslator()->trans('Invalid values', array(), 'Modules.Centerbanner.Admin');
            }
        } elseif (Tools::isSubmit('changeStatus')) {
            if (!Validate::isInt(Tools::getValue('id_slide'))) {
                $errors[] = $this->getTranslator()->trans('Invalid slide', array(), 'Modules.Centerbanner.Admin');
            }
            ...
            if (!Validate::isInt(Tools::getValue('active_slide')) ||
                ...
                $errors[] = $this->getTranslator()->trans('Invalid slide
                 state.', array(), 'Modules.Centerbanner.Admin');
            }
            /* Checks position */
            ...
            if (Tools::isSubmit('id_slide')) {
                if (!Validate::isInt(Tools::getValue('id_slide')) &&
                    ...
                    $errors[] = $this->getTranslator()->trans('Invalid 
                        slide ID', array(), 'Modules.Centerbanner.Admin');
                }
            }
            ...
            foreach ($languages as $language) {
                if (Tools::strlen(Tools::getValue('title_' . $language['id_lang'])) > 255) {
                    $errors[] = $this->getTranslator()->trans('The title is too 
                        long.', array(), 'Modules.Centerbanner.Admin');
                }
                ...
                if (Tools::strlen(Tools::getValue('url_' . $language['id_lang'])) > 255) {
                    $errors[] = $this->getTranslator()->trans('The URL is 
                        too long.', array(), 'Modules.Centerbanner.Admin');
                }
            ...
            if (Tools::strlen(Tools::getValue('url_' . $id_lang_default)) == 0) {
                $errors[] = $this->getTranslator()->trans('The URL is 
                    not set.', array(), 'Modules.Centerbanner.Admin');
            }
            if (!Tools::isSubmit('has_picture') && (!isset($_FILES['image_' . $id_lang_default]) ||
                ...
                $errors[] = $this->getTranslator()->trans('The image 
                    is not set.', array(), 'Modules.Centerbanner.Admin');
            }
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            switch ($shop_context) {
                case Shop::CONTEXT_ALL:
                    $res &= Configuration::updateValue('WBBANNER_SPEED', (int)Tools::getValue('WBBANNER_SPEED'));
                    $res &= Configuration::updateValue(
                        'WBBANNER_PAUSE_ON_HOVER',
                        (int)Tools::getValue(
                            'WBBANNER_PAUSE_ON_HOVER'
                    ...
                    $res &= Configuration::updateValue('WBBANNER_WRAP', (int)Tools::getValue('WBBANNER_WRAP'));
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue(
                                'WBBANNER_SPEED',
                ...
                case Shop::CONTEXT_GROUP:
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue(
                                'WBBANNER_SPEED',
                                (int)Tools::getValue('WBBANNER_SPEED'),
                                false,
                                $shop_group_id
                            );
                            $res &= Configuration::updateValue(
                                'WBBANNER_PAUSE_ON_HOVER',
                                (int)Tools::getValue('WBBANNER_PAUSE_ON_HOVER'),
                                false,
                                $shop_group_id
                            );
                            $res &= Configuration::updateValue(
                                'WBBANNER_WRAP',
                                (int)Tools::getValue('WBBANNER_WRAP'),
                                false,
                                $shop_group_id
                            );
                        }
            ...
            if (!$res) {
                $errors[] = $this->displayError($this->getTranslator()->trans('The configuration 
                    could not be updated.', array(), 'Modules.Centerbanner.Admin'));
            } else {
                Tools::redirectAdmin(
            ...
            $slide = new WbBanner((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new WbBanner((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    public function getWidgetVariables($hookName = null, array $configuration = null)
    {
        ...
        return array(
            'wbbanner' => array(
                'speed' => $config['WBBANNER_SPEED'],
                'pause' => $config['WBBANNER_PAUSE_ON_HOVER'] ? 'hover' : '',
                'wrap' => $config['WBBANNER_WRAP'] ? 'true' : 'false',
                'slides' => $slides,
            ),
        );
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute(
            'INSERT IGNORE INTO '._DB_PREFIX_.'wbbanner (id_wbbanner_slides, id_shop)
            SELECT id_wbbanner_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'wbbanner
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            ...
            FROM `'._DB_PREFIX_.'wbbanner_slides` hss, `'._DB_PREFIX_.'wbbanner` hs
            WHERE hss.`id_wbbanner_slides` = hs.`id_wbbanner_slides` 
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            'SELECT hs.`id_wbbanner_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
            FROM '._DB_PREFIX_.'wbbanner hs
            LEFT JOIN '._DB_PREFIX_.'wbbanner_slides hss ON (hs.id_wbbanner_slides = hss.id_wbbanner_slides)
            LEFT JOIN '._DB_PREFIX_.'wbbanner_slides_lang hssl ON (hss.id_wbbanner_slides = hssl.id_wbbanner_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(
                _MODULE_DIR_.'wbcenterbanner/views/img/'.$slide['image']
            );
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            FROM '._DB_PREFIX_.'wbbanner hs
            LEFT JOIN '._DB_PREFIX_.'wbbanner_slides hss ON (hs.id_wbbanner_slides = hss.id_wbbanner_slides)
            LEFT JOIN '._DB_PREFIX_.'wbbanner_slides_lang hssl ON (hss.id_wblider_slides = hssl.id_wbbanner_slides)
            WHERE hs.`id_wbbanner_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_wbbanner_slides` as id_slide
                FROM `'._DB_PREFIX_.'wbbanner` hs
                WHERE hs.`id_wbbanner_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = WbBanner::getAssociatedIdsShop((int)$slide['id_slide']);
            ...
            'form' => array(
                'legend' => array(
                    'title' => $this->getTranslator()->trans('Slide 
                        information', array(), 'Modules.Centerbanner.Admin'),
                    'icon' => 'icon-cogs'
                ),
                'input' => array(
                    array(
                    ...
                    array(
                        ...
                        'label' => $this->getTranslator()->trans('Target
                         URL', array(), 'Modules.Centerbanner.Admin'),
                        'name' => 'url',
                        'required' => true,
                        'lang' => true,
                    ),
                    array(
                        'type' => 'text',
                        'label' => $this->getTranslator()->trans('Caption', array(), 'Modules.Centerbanner.Admin'),
                        'name' => 'legend',
                        'lang' => true,
                    ),
                    array(
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    array(
                        'type' => 'text',
                        'label' => $this->getTranslator()->trans('Speed', array(), 'Modules.Centerbanner.Admin'),
                        'name' => 'WBBANNER_SPEED',
                        'suffix' => 'milliseconds',
                        'class' => 'fixed-width-sm',
                        'desc' => $this->getTranslator()->trans('The duration of the 
                            transition between two slides.', array(), 'Modules.Centerbanner.Admin')
                    ),
                    array(
                        'type' => 'switch',
                        'label' => $this->getTranslator()->trans('Pause on 
                            hover', array(), 'Modules.Centerbanner.Admin'),
                        'name' => 'WBBANNER_PAUSE_ON_HOVER',
                        'desc' => $this->getTranslator()->trans('Stop sliding when the 
                            mouse cursor is over the slideshow.', array(), 'Modules.Centerbanner.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
                    ...
                    array(
                        ...
                        'desc' => $this->getTranslator()->trans('Loop or stop after 
                            the last slide.', array(), 'Modules.Centerbanner.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
    ...
    public function getConfigFieldsValues()
    {
        ...
        return array(
            ...
            'WBBANNER_PAUSE_ON_HOVER' => Tools::getValue(
                'WBBANNER_PAUSE_ON_HOVER',
                Configuration::get(
                    'WBBANNER_PAUSE_ON_HOVER',
                    null,
    ...
    protected function getMultiLanguageInfoMsg()
    {
        return '<p class="alert alert-warning">'.
                    $this->getTranslator()->trans('Since multiple languages are activated on your shop, 
                        please mind to upload your image for each one of them', array(), 'Modules.Centerbanner.Admin').
                '</p>';
    }
    ...
    protected function getWarningMultishopHtml()
    {
        if (Shop::getContext() == Shop::CONTEXT_GROUP || Shop::getContext() == Shop::CONTEXT_ALL) {
            ...
            $this->getTranslator()->trans('You cannot manage slides items from a "All Shops" or a "G
                roup Shop" context, select directly the shop you want to edit', array(), 'Modules.Centerbanner.Admin') .
            '</p>';
        } else {
            return '';
        }
    ...
    protected function getShopContextError($shop_contextualized_name, $mode)
    {
        ...
        if ($mode == 'edit') {
            ...
            $this->trans('You can only edit this slide from the shop(s) 
                context: %s', array($shop_contextualized_name), 'Modules.Centerbanner.Admin') .
            '</p>';
        } else {
            return '<p class="alert alert-danger">' .
            $this->trans('You cannot add slides from a "All Shops" or a 
                "Group Shop" context', array(), 'Modules.Centerbanner.Admin') .
            '</p>';
        }
    }
        ...
        if (Shop::isFeatureActive()) {
            if (Shop::getContext() == Shop::CONTEXT_SHOP) {
                ...
                $shop_info = $this->trans('The modifications will be 
                    applied to all shops and shop groups', array(), 'Modules.Centerbanner.Admin');
            }
    ...
    protected function getSharedSlideWarning()
    {
        return '<p class="alert alert-warning">'.
                    $this->trans('This slide is shared with other shops! 
                        ...
                        modifications made here', array(), 'Modules.Centerbanner.Admin').
                '</p>';
    }
}
